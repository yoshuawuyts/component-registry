...
pub(crate) const SECTION_LABEL_CLASS: &str =
    "text-[11px] font-mono uppercase tracking-wider text-ink-500 mb-2";
...
pub(crate) fn row(label: &str, value: Value) -> Division {
    ...
    d.class("flex items-baseline justify-between gap-2 py-1");
    d.span(|s| {
        s.class("text-ink-500 text-[12px] shrink-0")
            .text(label.to_owned())
    });
    match value {
        Value::Text(text) => {
            d.span(|s| {
                s.class("text-ink-900 text-[12px] font-mono text-right")
                    .text(text)
            });
        }
        Value::Link { text, href } => {
            d.anchor(|a| {
                a.href(href)
                    .class("text-accent hover:underline font-mono text-[12px] text-right truncate")
                    .text(text)
            });
        }
    }
