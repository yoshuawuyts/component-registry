...
/// Auto-install a manifest key into the local project so that `wasm run`
/// can execute it directly without requiring an explicit `wasm install` step.
///
...
async fn auto_install(input: &str, offline: bool) -> miette::Result<()> {
    ...
    // Ensure the project skeleton exists so that `install` can succeed.
    // These mirror the directories and files produced by `wasm init`.
