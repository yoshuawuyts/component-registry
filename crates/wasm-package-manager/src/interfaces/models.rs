...
/// A WIT type package stored in the database.
///
...
impl WitType {
    /// Returns the primary-key ID of this WIT type.
    #[must_use]
    pub fn id(&self) -> i64 {
        self.id
    }
    ...
    /// Find a WIT type by package name and optional version.
    #[allow(dead_code)]
    pub(crate) fn find(
        conn: &Connection,
        package_name: &str,
    ...
    /// Return every WIT type, ordered by name then version.
    ...
    /// Return every WIT type together with its OCI reference string.
    ///
