...
--                    types (packages), worlds, imports, exports,
--                    and inter-type dependencies
...
-- ============================================================
-- WIT LAYER: Types, Worlds, Imports, Exports, Dependencies
--
-- A WIT type is the top-level publishable artifact in the
...
CREATE TABLE wit_type (
    -- Surrogate primary key.
    id INTEGER PRIMARY KEY,
    -- The WIT package name (namespace:name), e.g. "wasi:http".
    -- This is the primary search key for type lookups.
    package_name TEXT NOT NULL,
    -- The semver version string, e.g. "0.3.0".
    -- NULL if the type was recorded without a version.
    version TEXT,
    -- Human-readable description of this WIT type, used for
...
-- A named world defined inside a WIT type.  A world is a
...
CREATE TABLE wit_world (
    -- Surrogate primary key.
    id INTEGER PRIMARY KEY,
    -- The WIT type (package) this world is defined in.
    wit_type_id INTEGER NOT NULL,
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(wit_type_id, name),
    FOREIGN KEY (wit_type_id) REFERENCES wit_type(id)
        ON UPDATE NO ACTION ON DELETE CASCADE
);

-- Records that a world imports (depends on) an interface.
-- References are nominal: the declared name is always stored, and
-- resolution to a specific wit_type row is optional.
-- NULL resolved_type_id means "not yet resolved" or
...
CREATE TABLE wit_world_import (
    ...
    -- Optionally resolved to a specific wit_type row.
    -- NULL if resolution has not been performed or if multiple
    -- OCI sources publish the same type and the choice is
    -- ambiguous.
    resolved_type_id INTEGER,
    FOREIGN KEY (wit_world_id) REFERENCES wit_world(id)
        ON UPDATE NO ACTION ON DELETE CASCADE,
    FOREIGN KEY (resolved_type_id) REFERENCES wit_type(id)
        ON UPDATE NO ACTION ON DELETE SET NULL
);
...
-- Records that one WIT type depends on another at the package
...
CREATE TABLE wit_type_dependency (
    -- Surrogate primary key.
    id INTEGER PRIMARY KEY,
    -- The WIT type that declares this dependency.
    ...
    -- NULL if unresolved or ambiguous.
    resolved_type_id INTEGER,
    FOREIGN KEY (dependent_id) REFERENCES wit_type(id)
        ON UPDATE NO ACTION ON DELETE CASCADE,
    FOREIGN KEY (resolved_type_id) REFERENCES wit_type(id)
        ON UPDATE NO ACTION ON DELETE SET NULL
);
...
--   wit_type          — uq_wit_type(package_name, ...)
--   wit_world         — UNIQUE(wit_type_id, name)
--   wit_world_import  — uq_wit_world_import(wit_world_id, ...)
--   wit_world_export  — uq_wit_world_export(wit_world_id, ...)
--   wit_type_dep      — uq_wit_type_dependency(dependent_id, ...)
...
-- or all WIT type artifacts across the registry.
...
-- the COALESCE overhead of uq_wit_type.
CREATE INDEX idx_wit_type_name_version ON wit_type(package_name, version);
-- Find all WIT types extracted from a given OCI manifest,
...
-- that resolved their import to a specific wit_type row.
...
-- Reverse lookup on type dependencies: find all types
...
-- Reverse lookup on resolved type dependencies.
